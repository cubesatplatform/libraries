...
void loopWireA(arduino::MbedI2C &aWire) {
  byte error, address;
  int nDevices;
  ...
  for(byte address = 1; address < 127; address++ ) {
    Serial.print(".");
    aWire.beginTransmission(address);
    error = aWire.endTransmission();
    if (error == 0) {
      Serial.println();
...
uint8_t SCMDW::begin( void )
{
	...
	case I2C_MODE:
		Wire->begin();
		loopWireA(*Wire);
		Serial.print("Address    :");Serial.println(settings.I2CAddress);
		break;
	...
	//dummy read
	Serial.println(readRegister(SCMD_ID));
	for(char count=0;count<250;count++)
		//Serial.println(readRegister(SCMD_ID));
		Serial.println(readRegister(count));
	
	return readRegister(SCMD_ID);
}

//check if enumeration is complete
bool SCMDW::ready( void )
{
...
void SCMDW::setDrive( uint8_t motorNum, uint8_t direction, uint8_t level )
{
	//convert to 7 bit
	level = level >> 1;
...
void SCMDW::inversionMode( uint8_t motorNum, uint8_t polarity )
{
	uint8_t regTemp;
...
void SCMDW::bridgingMode( uint8_t driverNum, uint8_t bridged )
{
	uint8_t regTemp;
	//Select target register
	if( driverNum < 1 )
...
void SCMDW::getDiagnostics( SCMDDiagnostics &diagObjectReference )
{
...
//  SCMDDiagnostics &diagObjectReference -- Object to contain returned data
void SCMDW::getRemoteDiagnostics( uint8_t address, SCMDDiagnostics &diagObjectReference )
{
...
//
void SCMDW::resetDiagnosticCounts( void )
{
...
//
//  uint8_t address -- Address of slave to read.  Can be 0x50 to 0x5F for slave 1 to 16.
void SCMDW::resetRemoteDiagnosticCounts( uint8_t address )
{
...
uint8_t SCMDW::readRegister(uint8_t offset)
{
	//Return value
	uint8_t result;
	uint8_t numBytes = 1;
...
void SCMDW::writeRegister(uint8_t offset, uint8_t dataToWrite)
{
	switch (settings.commInterface)
	{
	case I2C_MODE:
...
//  uint8_t address -- Address of slave to read.  Can be 0x50 to 0x5F for slave 1 to 16.
//  uint8_t offset -- Address of data to read.  Can be 0x00 to 0x7F
uint8_t SCMDW::readRemoteRegister(uint8_t address, uint8_t offset)
{
	//while(busy());
...
//  uint8_t offset -- Address of data to write.  Can be 0x00 to 0x7F
//  uint8_t dataToWrite -- Data to write.
void SCMDW::writeRemoteRegister(uint8_t address, uint8_t offset, uint8_t dataToWrite)
{
	while(busy());
	writeRegister(SCMD_REM_ADDR, address);
	writeRegister(SCMD_REM_OFFSET, offset);
